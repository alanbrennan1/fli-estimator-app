...
<div className="p-4 border rounded bg-white">
  ...
  {/* 🆕 Cross-Section + Length Selector for Troughs */}
  {topLevelProduct === 'Troughs' && selectedProduct.startsWith('CT') && (
    <div className="mb-6 border border-gray-300 rounded-lg p-4 bg-gray-50 shadow-sm">
      <h4 className="text-xs font-bold uppercase text-gray-700 mb-3 tracking-wider border-b pb-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col">
          ...
          <select
            value={subProductInputs[selectedProduct]?.standardSection || ''}
            onChange={(e) => handleSubInputChange(selectedProduct, 'standardSection', e.target.value)}
            className="border rounded p-2 text-xs bg-white"
          >
            <option value="">Select</option>
            {Object.keys(groupedTroughs).map(section => (
        ...
        <div className="flex flex-col">
          ...
          <select
            value={subProductInputs[selectedProduct]?.standardLength || ''}
            onChange={(e) => handleSubInputChange(selectedProduct, 'standardLength', e.target.value)}
            className="border rounded p-2 text-xs bg-white"
            disabled={!subProductInputs[selectedProduct]?.standardSection}
          >
            <option value="">Select</option>
            {(groupedTroughs[subProductInputs[selectedProduct]?.standardSection] || []).map(len => (
